...
export default function Sidebar({ role = 'user' }: SidebarProps) {
  ...
  const menuItems = [
    ...
    { name: 'Data Sewa', href: '/dashboard/sewa', icon: CalendarClock },
  ...
  return (
    ...
    >
      {/* Brand Header */}
      <div className="flex h-16 items-center justify-between border-b border-teal-900 px-6">
      ...
      <div className="flex-1 space-y-7 overflow-y-auto px-4 py-6">
        <div className="space-y-1">
          {!collapsed && (
            <p className="px-2 text-xs font-semibold tracking-wider text-teal-400/70 uppercase">
              Menu Utama
            </p>
          )}
          {menuItems.map((item) => (
        ...
        {role === 'admin' && (
          <div className="space-y-1">
            {!collapsed && (
              <p className="px-2 text-xs font-semibold tracking-wider text-orange-400/70 uppercase">
                Administrator
              </p>
            )}
            {adminItems.map((item) => (
              <Link
                ...
                className={cn(
                  'flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-all duration-200',
                  isActive(item.href)
                    ? 'border-l-4 border-orange-500 bg-teal-800 text-white shadow-inner'
                    : 'text-slate-300 hover:bg-teal-900/60 hover:text-white'
                )}
              >
                <item.icon
                  className={cn(
                    'h-5 w-5 shrink-0',
                    isActive(item.href) ? 'text-orange-400' : 'text-slate-400'
                  )}
                />
                {!collapsed && <span>{item.name}</span>}
              </Link>
      ...
      {/* Footer Info */}
      {!collapsed && (
        <div className="border-t border-teal-900 bg-teal-950/40 p-4 text-center">
