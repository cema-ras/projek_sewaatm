...
export default function Navbar({ user }: NavbarProps) {
  ...
  const initial = nama ? nama.substring(0, 2).toUpperCase() : 'US'
  ...
  return (
    <header className="flex h-16 w-full items-center justify-between border-b border-slate-200 bg-white px-6 shadow-sm dark:border-slate-800 dark:bg-slate-950">
      ...
      <div className="flex items-center gap-4">
        ...
        {/* User Dropdown */}
        <DropdownMenu>
          <DropdownMenuTrigger className="flex cursor-pointer items-center gap-2 rounded-full border border-slate-200 px-3 py-1.5 transition-colors hover:bg-slate-50 focus:outline-none dark:border-slate-800 dark:hover:bg-slate-900">
            <div className="flex h-7 w-7 items-center justify-center rounded-full bg-teal-600 text-xs font-bold uppercase text-white shadow-sm">
              {initial}
            </div>
            <div className="hidden text-left sm:block">
              <p className="text-xs font-semibold leading-tight text-slate-700 dark:text-slate-300">
                {nama}
              </p>
          ...
          <DropdownMenuContent align="end" className="mt-1 w-56">
            ...
            <DropdownMenuItem
              ...
              className="cursor-pointer text-red-600 focus:bg-red-50 focus:text-red-700 dark:focus:bg-red-950/20 dark:focus:text-red-400"
            >
